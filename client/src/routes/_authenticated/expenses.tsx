...
export const Route = createFileRoute("/_authenticated/expenses")({ component: Expense });
...
function ExpenseDeleteButton({ id }: { id: number }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      toast("Success", { description: `Successfully deleted expense: ${id}` });
  ...
  return (
    <Button variant="outline" size="icon" onClick={() => mutation.mutate({ id })} disabled={mutation.isPending}>
      {mutation.isPending ? <Skeleton className="size-4" /> : <Trash className="size-4" />}
    </Button>
  );
}
